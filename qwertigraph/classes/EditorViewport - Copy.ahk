...
Gui MainGUI:Default
Gui, Tab, Editor
; Add regex search fields to the top
...
; Add the data ListView in the center 
Gui, Add, ListView, x12 y84 w826 h436 vEditorLV gEditorLV, Word|Form|Qwerd|Keyer|Chord|Chordable|Usage|Dictionary
...
; Add edit fields and controls to the bottom
; First the formal supplement fields
Gui, Add, Edit, x12  y520 w140 h20 vEditWord,
Gui, Add, Button, x152 y520 w20 h20 gEditorHone, H>
Gui, Add, Edit, x172 y520 w70  h20 vEditForm,
Gui, Add, Button, x242 y520 w20 h20 gEditorAutoQwerdForm, L>
Gui, Add, Edit, x262 y520 w90  h20 vEditQwerd,
Gui, Add, Button, x352 y520 w20 h20 gEditorAutoKey, K>
Gui, Add, Edit, x372 y520 w30  h20 vEditKeyer,
Gui, Add, Button, x402 y520 w20 h20 gEditorAutoChord, C>
Gui, Add, Edit, x422 y520 w50 h20 vEditChord,
Gui, Add, Edit, x472 y520 w80 h20 Disabled vEditChordable,
Gui, Add, Edit, x552 y520 w50  h20 vEditUsage,
Gui, Add, DropDownList, x602 y520 w236 r5 vEditDict, %dictionaryDropDown%
Gui, Add, Button, x838 y520 w90 h20 gEditorCommitEdit, Commit
; Then the uniform fields
Gui, Add, Edit, x262 y540 w90  h20 vEditUniformQwerd,
Gui, Add, Button, x352 y540 w20 h20 gEditorUniformAutoKey, K>
Gui, Add, Edit, x372 y540 w30  h20 vEditUniformKeyer,
Gui, Add, Button, x402 y540 w20 h20 gEditorUniformAutoChord, C>
Gui, Add, Edit, x422 y540 w50 h20 vEditUniformChord,
Gui, Add, Edit, x472 y540 w80 h20 Disabled vEditUniformChordable,
Gui, Add, Edit, x552 y540 w50  h20 vEditUniformUsage,
Gui, Add, DropDownList, x602 y540 w236 r5 vEditUniformDict, %dictionaryDropDown%
Gui, Add, Button, x838 y539 w90 h20 gEditorUniformCommitEdit, Commit
...
Gui, Add, Text, x838 y460 w90 h30 vEditsStatus, Edits: None
Gui, Add, Button, x838 y480 w90 h30 gEditorSaveDictionaries vSaveDictionaries , Save
...
EditorUniformAutoKey() {
	global editor
	editor.UniformAutoKey()
}
EditorUniformAutoChord() {
	global editor
	editor.AutoChord()
}
...
class EditorViewport
{
	...
	uniform_keyers := Array("","u","i","w","q","z","x","c","v","b","n","m","j","k","l","1","2","3","4","5","6","7")
	...
	autoQwerdForm() {
		...
		if (RegexMatch(word, "ing$")) {
			qwerd := RegexReplace(qwerd, "h$", "g")
		}
		if (RegexMatch(word, "ings$")) {
			qwerd := RegexReplace(qwerd, "h-s$", "gs")
		}
	...
	uniformAutoQwerdForm() {
        local
		Gui MainGUI:Default
		GuiControlGet form, , EditForm
		GuiControlGet word, , EditWord

		this.logEvent(3, "Generating uniform auto qwerd for " form)
		...
		; Consonant sets
		if (RegexMatch(form, "mn")) {
			qwerd := RegexReplace(qwerd, "mn", "mm")
		}
		if (RegexMatch(word, "x")) {
			qwerd := RegexReplace(qwerd, "es", "x")
		}
		...
		if (RegexMatch(word, "ings$")) {
			qwerd := RegexReplace(qwerd, "h-s$", "z")
		}
		if (RegexMatch(word, "qu")) {
			qwerd := RegexReplace(qwerd, "k", "q")
		}
        qwerd := RegexReplace(qwerd, "^r-e", "r")
		qwerd := RegexReplace(qwerd, "th", "h")
		qwerd := RegexReplace(qwerd, "ch", "c")
		qwerd := RegexReplace(qwerd, "sh", "z")
		...
		GuiControl, Text, EditUniformQwerd, %qwerd%
	...
	autoKey() {
		...
		newKeyer := this.getNextKeyer(qwerdKey, qwerd, word, 0)
	...
	uniformAutoKey() {
		...
		Gui MainGUI:Default
		GuiControlGet word, , EditWord
		GuiControlGet qwerd, , EditUniformQwerd
		GuiControlGet keyer, , EditUniformKeyer
		GuiControlGet dict, , EditUniformDict

		this.logEvent(3, "Mapping to dict " dict)
		dictionary := this.map.dictionaryShortToFullNames[dict]
		qwerdKey := dictionary "!!" word
		this.logEvent(3, "Seeking keyer for " qwerdKey)
		newKeyer := this.getNextKeyer(qwerdKey, qwerd, word, 1)
		this.logEvent(2, "Setting newKeyer to " newKeyer)
	...
	getNextKeyer(qwerdKey, qwerd, word, uniform) {
        local
		Gui MainGUI:Default
        ...
        ; Choose whether we are doing a standard keyer or a uniform keyer
        desired_keyers := uniform ? this.uniform_keyers : this.keyers
		for index, keyer in desired_keyers {
