        ...
        Switch key
        {
            ...
            case "Numlock":
                ...
                if (NumLockRequestTime - this.NumLockLastSetTime < 50) {
                    ...
                    ; I will set that property in the Aux Keyboard and watch it going forward 
                    this.engine.aux.NoOpRequired := 1
                }
                this.NumLockLastSetTime := A_TickCount
                
                if (this.engine.aux.NoOpRequired) {
                    this.logEvent(4, "No Op on NumLock required, so do nothing") 
                } else {
                    this.logEvent(4, "Toggle on NumLock required, so do it") 
